...
| `param_bounds` | `(0.8, 1.2)` | Multiplicative lower/upper factors for RC5 ADR context parameters. The lower factor must be between `0` and `1`; the upper factor must be at least `1`. `Tcn`/`Tan` keep their fixed `±5 K` window and `kd` keeps `(0, 0.1)`. | Narrow it for easier/safer contexts; widen it when ADR should search farther from nominal parameters. |
